...
void splitMatrix(const std::vector<double>& mtx, std::vector<double>* a,
                 ...
                 std::vector<double>* d, unsigned int size) {
...
void mergeMatrix(std::vector<double>* mtx, const std::vector<double>& a,
                 ...
                 const std::vector<double>& d, unsigned int size) {
  unsigned int sz = size / 2;
  for (unsigned int i = 0; i < sz; i++) {
    for (unsigned int j = i * sz; j < i * sz + sz; j++) mtx->push_back(a[j]);
    for (unsigned int j = i * sz; j < i * sz + sz; j++) mtx->push_back(b[j]);
  }
  for (unsigned int i = 0; i < sz; i++) {
    for (unsigned int j = i * sz; j < i * sz + sz; j++) mtx->push_back(c[j]);
    for (unsigned int j = i * sz; j < i * sz + sz; j++) mtx->push_back(d[j]);
  }
}
...
std::vector<double> toPowerOfTwoSize(const std::vector<double>& mtx,
  ...
  for (unsigned i = 0; i < sz; i++) {
    for (unsigned j = 0; j < sz; j++) res.push_back(mtx[j + i * sz]);
    for (unsigned j = sz; j < power; j++) res.push_back(0);
  }
  for (unsigned i = sz; i < power; i++) {
    for (unsigned j = 0; j < power; j++) res.push_back(0);
  }
  return res;
}
...
std::vector<double> toPowerOfTwoSize(const std::vector<double>& mtx,
  ...
  *power = 2;
  ...
  for (unsigned i = 0; i < sz; i++) {
    for (unsigned j = 0; j < sz; j++) res.push_back(mtx[j + i * sz]);
    for (unsigned j = sz; j < tpow; j++) res.push_back(0);
  }
  for (unsigned i = sz; i < tpow; i++) {
    for (unsigned j = 0; j < tpow; j++) res.push_back(0);
  }
  return res;
}
...
std::vector<double> strassenMultiplication_NoCastToPowerOfTwo_omp(
    const std::vector<double>& a_, const std::vector<double>& b_,
    unsigned int sz) {
  std::vector<double> res;
